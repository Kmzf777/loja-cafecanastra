...
- **Como mitigar:** Combinar a numeração antes de escrever qualquer .sql — a decisão foi reservar a faixa **0030+** (spec §2.9), e não 0022: buraco na numeração não incomoda o runner, e a folga absorve as worktrees inteiras sem depender da ordem de merge. E lembrar que a chave em canastra.migracoes é o NOME COMPLETO do arquivo: renomear uma migração já aplicada faz ela rodar de novo.
...
6. 6) DECIDIR E RODAR AS MIGRAÇÕES, COM NUMERAÇÃO COMBINADA. Alinhar produção (que está oito migrações atrás) e escrever a migração nova na faixa 0030+ (spec §2.9 — 0017 está triplamente disputado entre as worktrees), contendo os campos fiscais de produto. As três correções de privilégio deste item (REVOKE UPDATE em clientes com GRANT só de nome/telefone; SELECT por coluna em avaliacoes, com a RPC minhas_avaliacoes servindo a vitrine; REVOKE DELETE em config_loja) JÁ SAÍRAM, em 0031_correcoes_de_privilegio.sql. PORQUÊ: tela escrita contra coluna que não existe em produção quebra no deploy, e migração é o item de maior lead time (depende de janela e de merge das worktrees).
...
- NUMERAÇÃO DAS MIGRAÇÕES E ORDEM DE MERGE DAS WORKTREES: 0017 está ocupado em melhor-envio e em whatsapp-bot (que vai até 0021). Alguém precisa decidir a ordem de merge; o número inicial do painel já está decidido e é 0030+ (spec §2.9), que não depende dessa ordem — o runner aborta em número repetido e a chave de controle é o nome completo do arquivo.
